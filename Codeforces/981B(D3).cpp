...
int traverseDiagonallyOpposite(const vector<vector<int>>& matrix) {
    int n = matrix.size();
    if (n == 0) return 0;
    int m = matrix[0].size();
    ...
    for (int row = n - 1; row >= 0; row--) {
        ...
        while (startRow < n && startCol < m) {
            if (matrix[startRow][startCol] < 0) {
                maxAbsValue = max(maxAbsValue, -matrix[startRow][startCol]);
            }
            startRow++;
            startCol++;
        }

        if (maxAbsValue != INT_MIN) {
            cnt += maxAbsValue;
        }
    }
...
int main() {
    ...
    while (t--) {
        ...
        cout << traverseDiagonallyOpposite(matrix) << endl;
    }
