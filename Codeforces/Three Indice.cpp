...
int minFinder(vector<int>& v, int start, int end) {
    int mini = INT_MAX;
    int idx = start;
...
int main()
{
    ...
    while(t--){ 
        ...
        while(mid<n-1){
            int minLeft = minFinder(v,0,mid-1);
            int minRight = minFinder(v,mid+1,n-1);
            if(v[minLeft]<v[mid] && v[minRight]<v[mid]){
                cout << "YES" << endl;
                cout << minLeft+1 <<" "<<mid+1 <<" "<< minRight+1 << endl;
                flag = false;
                break;
            }
            else{
