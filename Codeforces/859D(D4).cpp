...
void scanVec(vector<ll>& v, int size) { for (ll i = 0; i < size; i++) { ll x; cin >> x; v.push_back(x); } }
...
int main()
{
    ...
    while(t--){ 
        ...
        while(q--){
            ...
            ll rangedSum = prefixSum[b] - prefixSum[a-1];
            ll newSum = totalSum - rangedSum + c*(b-a+1);
